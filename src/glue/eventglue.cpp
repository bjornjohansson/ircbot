...
private:
    ...
    typedef std::map<Irc::Command::Command,
		     Client::EventReceiverHandle> EventReceiverHandleContainer;
...
void EventGlue::OnEvent(const std::string& server,
			const Irc::Message& message)
{
    const Irc::Command::Command& command = message.GetCommand();
    for(FunctionContainer::iterator func = eventFunctions_[command].begin();
	func != eventFunctions_[command].end();
	++func)
    {
...
int EventGlue::RegisterForEvent(lua_State* lua)
{
	...
	EventReceiverHandleContainer::iterator r =
	    eventReceiverHandles_.find(command);

	if ( r == eventReceiverHandles_.end() )
	{
	    Client::EventReceiverHandle handle = 
		client_->RegisterForEvent(command,
					  boost::bind(&EventGlue::OnEvent,
						      this, _1, _2));
...
EventGlue::StringContainerPtr
EventGlue::CallEventHandlers(FunctionStatePair functionStatePair,
			     const std::string& server,
			     const Irc::Message& message)
{
	...
	for(Irc::Message::const_iterator param = message.begin();
	    param != message.end();
	    ++param, ++argCount)
	{
